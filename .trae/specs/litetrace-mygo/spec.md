...
实现一个纯 Go 语言的 ftrace 追踪工具（简易版 trace-cmd），通过直接读写 tracefs 文件系统来操作 Linux 内核追踪功能，无需依赖任何外部二进制程序。

## What Changes

* 创建完整的 Go 项目结构，采用领域驱动设计

* 实现基于 Cobra 框架的 CLI 命令行接口

* 实现 ftrace 核心引擎，支持 function/function\_graph 追踪器
...
* Affected specs: 全部 9 个阶段的开发任务

* Affected code: 项目全部源代码文件

## ADDED Requirements

### Requirement: 核心追踪功能

系统 SHALL 提供纯 Go 实现的 ftrace 追踪能力

#### Scenario: 开启 function 追踪

* **WHEN** 用户执行 `litetrace run --tracer function --filter vfs_read --duration 5s --output trace.txt`

* **THEN** 系统向 tracefs 写入配置，启动追踪，并在指定时间后导出结果

### Requirement: 流式搜索功能

系统 SHALL 提供内核函数流式搜索能力

#### Scenario: 搜索函数

* **WHEN** 用户执行 `litetrace search "^vfs_"`

* **THEN** 系统返回匹配的内核函数列表（最多 100 条）
...
* **WHEN** 用户执行 `litetrace wizard`

* **THEN** 系统通过终端交互引导用户完成追踪配置

### Requirement: TUI 实时监控

系统 SHALL 提供 TUI 实时监控界面

#### Scenario: 启动 TUI

* **WHEN** 用户执行 `litetrace tui`
...
* **WHEN** 用户执行 `litetrace status`

* **THEN** 系统显示当前 tracefs 配置状态
...
* github.com/rivo/tview TUI 组件

* github.com/gdamore/tcell/v2 终端控制

* github.com/manifoldco/promptui 交互式问答
